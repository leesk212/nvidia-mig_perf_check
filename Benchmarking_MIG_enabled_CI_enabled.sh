...
echo "logging" > ./result_of_mig_ci_enabled.txt
...
do
	...
	echo "logging" > ./mig_ci_enabled/mig_ci_enabled_${size}.txt
	...
	for number in {1..10} 
			...
			echo " Latency: $diff s"
			echo "$diff" >> ./mig_ci_enabled/mig_ci_enabled_${size}.txt

		done
		
	python3 avg.py mig_ci_enabled_${size}.txt >> ./result_of_mig_ci_enabled.txt
